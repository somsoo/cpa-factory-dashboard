...
이 문서는 '원페이지 사이트(유틸리티 앱)'를 서브에이전트(Subagents) 멀티 시스템을 활용하여 100% 무인화로 기획, 개발, 마케팅하는 표준 작업 지침(SOP)입니다.
...
## 2. 8-Agent 어벤져스 파이프라인
원페이지 제작 지시가 떨어지면, 다음 8명의 에이전트를 순차적/병렬적으로 투입하여 프로젝트를 완수합니다.
...
- 역할: 사람 냄새나는 텍스트 작성.
- 검증: 서브 페이지에 들어갈 상세 가이드 문서 및 유저의 클릭을 유도하는 매력적인 버튼 문구(Microcopy) 작성.

### Step 5: 💻 프론트엔드 개발자 (Frontend Developer)
- 역할: 코딩 및 UI/UX 구현.
- 검증: 무거운 프레임워크 배제(순수 HTML/CSS/VanillaJS 사용), 모바일 최적화, 그리고 **애드센스/쿠팡 파트너스 명당 위치 절대 사수**.

### Step 6: 🎨 SEO & 에셋 디자이너 (Asset Manager)
- 역할: 시각 및 기술적 SEO 자산 생성.
- 검증: 파비콘 및 OG 이미지 생성, `sitemap.xml`, `robots.txt`, 그리고 JSON-LD 구조화 데이터 코드 주입.

### Step 7: 🧐 QA 리뷰어 (QA Reviewer)
- 역할: 최종 산출물 품질 관리.
- 검증: 모바일 환경 레이아웃 깨짐 테스트, 규칙 준수 여부 신랄하게 비판 후 반려 또는 승인.

### Step 8: 📢 그로스 해커 (Growth Hacker)
- 역할: 배포 직후 초기 트래픽 폭발 유도.
- 검증: 네이버 지식인, 카페, Threads 등 타겟 커뮤니티에 '사람이 직접 추천하는 듯한' 자연스러운 바이럴 홍보 원고 3종 세트 작성.
